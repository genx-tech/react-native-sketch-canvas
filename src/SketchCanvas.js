...
class SketchCanvas extends React.Component {
    ...
    componentDidUpdate() {
        if (!deepEqual(this.props.text, this.state.text)) {
            this.setState({
                text: this._processText(
                    this.props.text
                        ? this.props.text.map((t) => ({...t}))
                        : null,
                ),
            });
        }
    ...
    redo() {
        let id = null;
        if (this._undonePaths.length > 0) {
            const path = this._undonePaths[this._undonePaths.length - 1];
            console.log(JSON.stringify(path, null, 1));
            id = path.path.id;
            this.addPath(path);
            this._undonePaths.splice(this._undonePaths.length - 1, 1);
            console.log(this._undonePaths.length);
        }
        return id;
    }
    ...
    deletePath(id) {
        const path = this._paths.find((_) => _.path.id === id);
        if (path !== null) {
            this._undonePaths.push({...path});
            console.log(JSON.stringify(this._undonePaths, null, 1));
        }
    ...
    validateDrawingState(evt, gestureState) {
        ...
        if (
            this.touchStartTime + this.props.startToDrawDelay <
            new Date().getTime()
        ) {
            this.startDrawing(evt, gestureState);
            return true;
