...
export default class ResponsiveView extends Component {
    ...
    componentWillMount() {
        ...
        this.panGesture = PanResponder.create({
            onStartShouldSetPanResponder: (evt, gestureState) =>
                gestureState.numberActiveTouches === requiredTouches,
            ...
            onPanResponderMove: (e, gestureState) => {
                if (e.nativeEvent?.changedTouches?.length === 2) {
                    ...
                    if (this.state.initialDistance) {
                        const deltaZoom =
                            (distance / this.state.initialDistance - 1) / 2;
    ...
    checkBounds(gestureState) {
        ...
        if (this.dual.x._value < -this.$scrollerXBound * this.zoom) {
            obj.x = -this.$scrollerXBound * this.zoom;
        }
        ...
        if (this.dual.y._value < -this.$scrollerYBound * this.zoom) {
            obj.y = -this.$scrollerYBound * this.zoom;
        }
            ...
            <Animated.View style={styles.viewport} {...this.panGesture.panHandlers}>
                <Animated.View
                    style={[
                        ...
                        this.dual.getLayout(),
                        {transform: [{scaleX: this.zoom}, {scaleY: this.zoom}]},
                    ]}>
                    {this.props.children}
                </Animated.View>
            </Animated.View>
